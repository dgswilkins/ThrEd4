...
void formForms::refrm() {
  ...
  FormDataSheet = CreateWindowEx(0,
                                 ...
                                 ButtonWidthX3 + 3,
                                 3,
                                 LabelWindowSize.cx + DLGW + ValueWindowSize.cx + DLGN,
                                 (LabelWindowSize.cy * gsl::narrow<LONG>(formMenuEntryCount)) + DLGW,
                                 ThrEdWindow,
                                 nullptr,
                                 ThrEdInstance,
                                 nullptr);
...
void formForms::sidwnd(HWND wnd, HWND& sideMessageWindow) {
  ...
  sideMessageWindow = CreateWindowEx(0,
                                     ...
                                     windowRect.right - ThredWindowOrigin.x + 4,
                                     windowRect.top - ThredWindowOrigin.y - 1,
                                     ButtonWidthX3,
...
void formForms::prfmsg() {
  ...
  PreferencesWindow = CreateWindowEx(0,
                                     ...
                                     3,
                                     windowWidth,
                                     (LabelWindowSize.cy * PRFLINS) + DLGW,
                                     ThrEdWindow,
                                     nullptr,
                                     ThrEdInstance,
                                     nullptr);
...
void formForms::wavfrm() {
  ...
  *itVertex = currentPosition;
  ...
  ClosestFormToCursor = wrap::toUnsigned(formList.size() - 1U);
  FormMoveDelta       = F_POINT {};
  form::setNewFormVertexCount(vertexCount + 1U);
  form::setmfrm(ClosestFormToCursor);
  form::mdufrm();
}
