...
void thred::internal::ritbal() {
	...
	if (!BalaradName0->empty() && !BalaradName1->empty() && (PCSHeader.stitchCount != 0u)) {
		...
		auto balaradStitch = std::vector<BALSTCH>{};
		balaradStitch.resize(gsl::narrow<size_t>(PCSHeader.stitchCount) + 2u);
...
void thred::internal::dubuf(char* const buffer, unsigned& count) {
	...
	if (!FormList->empty()) {
		...
		for (auto& srcForm : (*FormList)) {
			...
			auto vertexIt = FormVertices->cbegin() + srcForm.vertexIndex;
			...
			if (clip::isclp(srcForm)) {
				auto offsetStart = ClipPoints->cbegin() + srcForm.angleOrClipData.clip;
			...
			if (clip::iseclpx(srcForm)) {
				auto       offsetStart = ClipPoints->cbegin() + srcForm.borderClipData;
...
void thred::internal::savmap() {
	if (PCSBMPFileName[0] != 0) {
		...
		if (GetSaveFileName(&OpenBitmapName)) {
			...
			buffer.resize((gsl::narrow<size_t>(BitmapWidth) * BitmapHeight * 3) + 1);
	...
	if (!StateMap.testAndReset(StateFlag::BZUMIN)) {
		do {
			...
			if (StateMap.test(StateFlag::FRMPSEL)) {
				auto vertexIt = FormVertices->cbegin() + (*FormList)[ClosestFormToCursor].vertexIndex;
					...
					if (ThrEdClipPointer != nullptr) {
						...
						for (auto& selectedForm : (*SelectedFormList)) {
							...
							if (clip::isclpx(selectedForm)) {
								auto offsetStart = ClipPoints->cbegin() + SelectedForm->angleOrClipData.clip;
							...
							if (clip::iseclp(selectedForm)) {
								auto offsetStart = ClipPoints->cbegin() + SelectedForm->borderClipData;
...
void thred::internal::drwStch() {
	...
	auto linePoints = std::vector<POINT>{};
	linePoints.resize(gsl::narrow<size_t>(stitchCount) + 2);
	FillRect(StitchWindowMemDC, &StitchWindowClientRect, BackgroundBrush);
	thred::duzrat();
