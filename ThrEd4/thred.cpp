...
void thi::sachk() {
  for (auto iForm = 0U; iForm < wrap::toUnsigned(FormList->size()); ++iForm) {
	if (auto const& form = FormList->operator[](iForm); form.type == SAT && form.satinGuideCount != 0U) {
	  auto itGuide = wrap::next(SatinGuides->cbegin(), form.satinGuideIndex);
	  for (auto iGuide = 0U; iGuide < form.satinGuideCount; ++iGuide) {
...
auto APIENTRY wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPTSTR lpCmdLine, _In_ int32_t nShowCmd)
  ...
  if (auto const hResult = CoInitializeEx(nullptr, COINIT_APARTMENTTHREADED | COINIT_DISABLE_OLE1DDE);
	...
	if (RegisterClassEx(&winClass)) {
	  ...
	  LRUPtr                    = &Instance->LRUMenuId;
	  MenuInfo                  = &Instance->MenuInfo;
	  MsgBuffer                 = &Instance->MsgBuffer;
	  NearestPixel              = &Instance->NearestPixel;
	  ...
	  SideWindow                = &Instance->SideWindow;
	  SideWindowEntryBuffer     = &Instance->SideWindowEntryBuffer;
	  SortBuffer                = &Instance->SortBuffer;
	  ...
	  ThreadSizeWin             = &Instance->ThreadSizeWin;
	  ThumbnailSearchString     = &Instance->ThumbnailSearchString;
	  Thumbnails                = &Instance->Thumbnails;
	  TracedEdges               = &Instance->TracedEdges;
	  TracedMap                 = &Instance->TracedMap;
	  UndoBuffer                = &Instance->UndoBuffer;
	  UserColorWin              = &Instance->UserColorWin;
	  UserFlagMap               = &Instance->UserFlagMap;
	  UserPen                   = &Instance->UserPen;
	  ValueWindow               = &Instance->ValueWindow;
	  VersionNames              = &Instance->VersionNames;
