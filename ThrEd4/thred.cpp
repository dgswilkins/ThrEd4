...
auto     StitchArrow = std::array<POINT, arrowPoints> {}; // arrow for selected stitch
...
void thred::internal::ritfnam(std::wstring const& designerName) {
  ...
  if (iName == NameOrder.size()) {
	while (NameDecoder[tmpName[NameOrder.size() - 1U]] != 0U) {
	  tmpName[NameOrder.size() - 1U] = form::psg() & BYTMASK;
	}
  }
  for (iName = 0U; iName < NameOrder.size(); ++iName) {
...
auto thred::internal::pxchk(double pixelSize) -> uint16_t {
  constexpr auto minLimit = 1.0;
  constexpr auto maxLimit = 20.0;
...
void thred::internal::frmcalc(uint32_t& largestStitchIndex, uint32_t& smallestStitchIndex) {
  ...
  if ((form.fillType != 0U) || (form.edgeType != 0U)) {
	...
	constexpr auto digitLimit = 10000.0; // value that represents the max width that can be displayed
...
void thred::internal::ritini() {
  ...
  constexpr auto thresholdMax = 9.0;
  constexpr auto thresholdMin = 0.0;
...
void thred::internal::chk1col() {
  ...
  for (auto iColorChange = 0U; iColorChange < thred::maxColor(); ++iColorChange) {
	if (ColorChangeTable->    operator[](gsl::narrow_cast<size_t>(iColorChange) + 1U).stitchIndex -
...
auto thred::internal::savePCS(fs::path const* auxName, std::vector<fPOINTATTR>& saveStitches) -> bool {
  auto flag = true;
  if (nullptr != auxName) {
	// NOLINTNEXTLINE(readability-qualified-auto)
	...
	if (fileHandle == INVALID_HANDLE_VALUE) {
	  ...
	  do {
		...
		if (UserFlagMap->test(UserFlag::BSAVOF)) {
		  ...
		  if (FALSE ==
		      WriteFile(fileHandle, static_cast<LPCVOID>(PCSBMPFileName), sizeof(PCSBMPFileName) - 1U, &bytesWritten, nullptr)) {
			displayText::riter();
			flag = false;
			break;
		  }
...
#endif
	default: {
	  flag = savePCS(AuxName, saveStitches);
	}
  }
  if (flag) {
...
void thred::internal::closPnt() {
  ...
  gapToNearest.resize(NERCNT, BIGDOUBLE); // to a mouse click
  NearestPoint.fill(-1);
  ...
  for (auto iColor = 0U; iColor < thred::maxColor(); ++iColor) {
	...
	auto const            iStitch1 =
	    ColorChangeTable->operator[](gsl::narrow_cast<size_t>(iColor) + 1U).stitchIndex - iStitch0;
...
auto thred::internal::closPnt1(uint32_t& closestStitch) -> bool {
  ...
  auto const stitchPoint     = thred::pxCor2stch(Msg.pt);
  auto       distanceToClick = BIGDOUBLE;
  if (StateMap->test(StateFlag::HID)) {
	for (auto iColor = 0U; iColor < thred::maxColor(); ++iColor) {
	  if (ColorChangeTable->operator[](iColor).colorIndex == ActiveColor) {
...
auto thred::internal::closlin() -> uint32_t {
  ...
  for (auto iChange = 0U; iChange < thred::maxColor(); ++iChange) {
	...
	if (ColorChangeTable->operator[](gsl::narrow_cast<size_t>(iChange) + 1U).stitchIndex ==
	    StitchBuffer->size()) {
	  --stitchCount;
	}
...
void thred::internal::lodclp(uint32_t iStitch) {
  ...
  for (auto& clip : *ClipBuffer) {
	StitchBuffer->operator[](iStitch++) =
	    fPOINTATTR {clip.x + ClipOrigin.x,
	                clip.y + ClipOrigin.y,
...
void thred::savclp(CLPSTCH& destination, fPOINTATTR const& source, uint32_t led) {
  ...
  destination.fx   = wrap::floor<uint8_t>(fractional * fractionalFactor);
  destination.x    = gsl::narrow<uint16_t>(integer);
  destination.spcx = 0;
  fractional       = std::modf(source.y - LowerLeftStitch.y, &integer);
  destination.fy   = wrap::floor<uint8_t>(fractional * fractionalFactor);
...
#pragma warning(suppress : 26429) // f.23 Symbol is never tested for nullness, it can be marked as not_null
	...
	clipBuffer.emplace_back(gsl::narrow_cast<float>(clipStitchData[0].x) +
	                            gsl::narrow_cast<float>(clipStitchData[0].fx) / fractionalFactor,
	                        gsl::narrow_cast<float>(clipStitchData[0].y) +
	                            gsl::narrow_cast<float>(clipStitchData[0].fy) / fractionalFactor,
	                        0U);
	...
	for (auto iStitch = 1U; iStitch < clipSize; ++iStitch) {
	  clipBuffer.emplace_back(gsl::narrow_cast<float>(clipStitchData[iStitch].x) +
	                              gsl::narrow_cast<float>(clipStitchData[iStitch].fx) / fractionalFactor,
	                          gsl::narrow_cast<float>(clipStitchData[iStitch].y) +
	                              gsl::narrow_cast<float>(clipStitchData[iStitch].fy) / fractionalFactor,
		...
		if (bytesRead == fileSize) {
		  ...
		  for (auto iPCSStitch = 0U; iPCSStitch < pcsStitchCount; ++iPCSStitch) {
			if (pcsStitchBuffer[iPCSStitch].tag == 3) {
			  ...
			  (*StitchBuffer)
			      .emplace_back(fPOINTATTR {
			          gsl::narrow_cast<float>(pcsStitchBuffer[iPCSStitch].x) +
...
void thred::internal::longer() {
  ...
  auto const rangeEnd = ((gsl::narrow_cast<size_t>(SelectedRange.finish) + 1U) < StitchBuffer->size())
  ...
  if (StateMap->test(StateFlag::INIT)) {
	...
	if (StateMap->test(StateFlag::ZUMED)) {
	  ...
	  for (auto iColor = 0U; iColor < thred::maxColor(); ++iColor) {
		if (StateMap->test(StateFlag::HID)) {
		  if (ColorChangeTable->operator[](iColor).colorIndex != ActiveColor) {
			stitchCount = ColorChangeTable->operator[](gsl::narrow_cast<size_t>(iColor) + 1U).stitchIndex -
			              ColorChangeTable->operator[](iColor).stitchIndex;
			auto const                      stitchIt =
			    std::next(StitchBuffer->begin(), ColorChangeTable->operator[](iColor).stitchIndex);
			for (auto iStitch = 0U; iStitch < stitchCount; ++iStitch) {
			...
			if ((ActiveLayer == 0U) || (layer == 0U) || (layer == ActiveLayer)) {
			  if (stitchIt[iStitch].x >= ZoomRect.left && stitchIt[iStitch].x <= ZoomRect.right &&
				...
				if (StateMap->testAndReset(StateFlag::LININ)) {
				  ...
				  wrap::Polyline(
				      StitchWindowMemDC, linePoints.data(), gsl::narrow<uint32_t>(linePoints.size()));
				  linePoints.clear();
				}
				else {
				  if (iStitch != 0U) {
