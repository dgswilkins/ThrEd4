...
void satin::spltsat(uint32_t guideIndex) {
	...
	formList.insert(std::next(formList.cbegin(), ClosestFormToCursor), form);
	...
	form = formList.operator[](ClosestFormToCursor); // insert may have invalidated
	...
	const auto offset = form.satinOrAngle.guide + iNewGuide - 1U;
...
void satin::internal::satclos() {
	auto& form = FormList->operator[](ClosestFormToCursor);
	const auto initialGuideCount = form.satinGuideCount;
...
void satin::internal::satcpy(FRMHED& form, const std::vector<SATCON>& source, uint32_t size) {
	const auto& currentFormGuides = form.satinOrAngle.guide;
...
void satin::satadj(FRMHED& form) {
	...
	const auto savedGuideCount = form.satinGuideCount;
	...
	if (CurrentFormGuidesCount > iDestination) {
		...
		CurrentFormGuidesCount = form.satinGuideCount = iDestination;
	}
	...
	if ((endGuide != 0U) || ((form.attribute & FRMEND) != 0U)) {
		...
		if (endGuide != 0U) {
			...
			satinMap.set((wrap::toSize(endGuide) + 1U) % VertexCount);
		}
	...
	if (CurrentFormGuidesCount != 0U && endGuide != 0) {
		satinMap.reset();
		...
		CurrentFormGuidesCount = form.satinGuideCount = (iGuide);
		satinMap.reset();
		...
		CurrentFormGuidesCount = form.satinGuideCount = iGuide;
		if (endGuide != 0U) {
			const auto end0 = (VertexCount - endGuide - 2U);
			const auto end1 = (endGuide - 2U);
			...
			form.satinGuideCount = CurrentFormGuidesCount;
		}
	}
	else {
		// sort the guides
...
void satin::delcon(FRMHED& form, uint32_t GuideIndex) {
	const auto offset = form.satinOrAngle.guide + GuideIndex;
...
/*
	...
	if (ClosestVertexToCursor < endGuide) {
		endGuide--;
	}
*/
...
void satin::internal::satends(const FRMHED& form, uint32_t isBlunt, float width) {
		...
		if (VertexCount > 2) {
			...
			if (!FormList->empty()) {
				...
				if (currentForm.type == FRMLINE) {
					...
					newForm.vertexIndex         = thred::adflt((VertexCount * 2U) + 2U);
					auto vertexIt            = std::next(FormVertices->begin(), newForm.vertexIndex);
					vertexIt[0].x            = OutsidePoints->front().x;
					vertexIt[iNewVertex++].y = OutsidePoints->front().y;
...
void satin::slbrd(const FRMHED& form) {
...
void satin::internal::satmf(const FRMHED& form, const std::vector<float>& lengths) {
	auto start = 0U;
	...
	auto endGuideIndex = CurrentFormGuidesCount;
	if (endGuideIndex != 0U) {
		endGuideIndex--;
	}
	...
	if (endGuide != 0U) {
		...
		if (guideIt[endGuideIndex].finish - guideIt[endGuideIndex].start > 2) {
			auto length
				= (lengths[guideIt[endGuideIndex].finish] - lengths[guideIt[endGuideIndex].start]) / 2 + lengths[guideIt[endGuideIndex].start];
...
void satin::internal::sfn(const FRMHED& form, uint32_t startVertex) {
	for (auto iVertex = 0U; iVertex < form.vertexCount; iVertex++) {
...
void satin::satout(const FRMHED& form, float satinWidth) {
	if (VertexCount != 0U) {
		form::duangs(form);
		...
		si::outfn(form, VertexCount - 1, 0, satinWidth);
...
void satin::sbrd(const FRMHED& form) {
	const auto savedSpacing = LineSpacing;
...
void satin::internal::outfn(const FRMHED& form, uint32_t start, uint32_t finish, float satinWidth) {
	auto xOffset = 0.0F;
	auto yOffset = 0.0F;
