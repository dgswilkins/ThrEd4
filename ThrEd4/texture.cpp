...
void setxclp(FRM_HEAD const& form) {
  ...
  std::ranges::transform(angledFormVertices, angledFormVertices.begin(), [editorOffset](auto& vertex) noexcept -> auto {
...
void txtdel() {
  ...
  if (!selectedTexturePointsList.empty()) {
	...
	auto index = 0U;
	std::erase_if(tempTexturePoints, [&index, texturePointsMap]([[maybe_unused]] auto point) -> auto {
...
void texture::deltx(uint32_t const formIndex) {
  ...
  auto const flagShared =
      std::any_of(formList.begin(),
                  itForm,
                  [currentIndex](const auto& current) noexcept -> auto {
	                return current.isTexture() && current.texture.index == currentIndex;
                  }) ||
      std::any_of(std::next(itForm), formList.end(), [currentIndex](const auto& current) noexcept -> auto {
	    return current.isTexture() && current.texture.index == currentIndex;
      });
...
auto texture::textureInit() -> uint32_t {
  ...
  try {
	...
	outDebugString(L"Memory allocation failure in textureInit\n");
	return EXIT_FAILURE;
  }
  catch (std::exception const& e) {
	auto const errorMsg = utf::utf8ToUtf16(std::string(e.what()));
	outDebugString(L"Exception caught in textureInit: {}\n", errorMsg);
	return EXIT_FAILURE;
  }
  catch (...) {
	outDebugString(L"Unknown exception caught in textureInit\n");
	return EXIT_FAILURE;
  }
  return EXIT_SUCCESS;
}
