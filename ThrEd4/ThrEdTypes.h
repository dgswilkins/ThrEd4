...
constexpr float    LOWFLOAT = std::numeric_limits<float>::lowest(); // lowest number for single precision float stuff
constexpr double   BIGDBL = std::numeric_limits<double>::max(); // large number for double precision float stuff
constexpr double   LOWDBL = std::numeric_limits<double>::lowest(); // lowest number for double precision float stuff
constexpr auto     BIGLONG  = std::numeric_limits<LONG>::max();    // large number for LONG stuff
constexpr auto     LOWLONG  = std::numeric_limits<LONG>::lowest(); // lowest number for LONG stuff
constexpr auto     BIGDWORD = std::numeric_limits<DWORD>::max();   // large number for DWORD stuff
constexpr auto     LOWDWORD = std::numeric_limits<DWORD>::lowest(); // lowest number for DWORD stuff
constexpr auto     BIGUINT = std::numeric_limits<uint32_t>::max(); // large number for uint32 stuff
constexpr auto BIGINT32 = std::numeric_limits<int32_t>::max(); // large number for int32 stuff
constexpr auto BIGSIZE = std::numeric_limits<size_t>::max(); // large number for size stuff
