...
void form::frmlin(unsigned int vertices, unsigned int vertexCount) {
	if (VertexCount != 0u) {
		...
		for (auto iVertex = 0u; iVertex < vertexCount; iVertex++) {
			formLines.push_back(
			    POINT { wrap::round<long>((vertexIt[iVertex].x - ZoomRect.left) * ZoomRatio.x),
...
void form::frmlin(const std::vector<fPOINT>& vertices) {
	...
	if (vertexMax != 0u) {
		...
		for (auto iVertex = 0u; iVertex < vertexMax; iVertex++) {
			formLines.push_back(
			    POINT { wrap::round<long>((vertices[iVertex].x - ZoomRect.left) * ZoomRatio.x),
...
void form::sRct2px(const fRECTANGLE& stitchRect, RECT& screenRect) {
	...
	screenRect.bottom = wrap::round<long>((StitchWindowClientRect.bottom) - (stitchRect.bottom - ZoomRect.bottom) * ZoomRatio.y + 0.5);
}
...
void form::internal::ritapbrd() {
	if (!OSequence->empty()) {
		InterleaveSequenceIndices->emplace_back(INSREC {
		    TYPMSK, gsl::narrow<unsigned int>(SelectedForm->borderColor) >> 4, wrap::toUnsigned(InterleaveSequence->size()), I_AP });
		form::chkseq(true);
	}
}
...
bool form::internal::clpnxt(const std::vector<CLPSEG>& clipSegments, const std::vector<LENINFO>& sortedLengths, unsigned sind) {
	auto       index        = 1u;
	const auto indexDoubled = wrap::toUnsigned(clipSegments.size()) << 1;
...
bool form::internal::lnclos(std::vector<unsigned>& groupIndexSequence,
	...
	if (lineEndPoint0 != nullptr) {
		auto count0 = (groupIndexSequence[wrap::toSize(group0) + 1u] - groupIndexSequence[group0]) >> 1;
		auto index0 = 0u;
		...
		if (count0 != 0u) {
			auto count1 = (groupIndexSequence[wrap::toSize(group1) + 1u] - groupIndexSequence[group1]) >> 1;
			auto index1 = 0u;
...
bool form::chkfrm(std::vector<POINT>& stretchBoxLine, double& xyRatio) {
	...
	for (auto iControl = 0u; iControl < 10; iControl++) {
		...
		if (minimumLength < CLOSENUF) {
			...
			for (auto iCorner = 0u; iCorner < 4; iCorner++) {
				stretchBoxLine[iCorner] = formControls[wrap::toSize(iCorner) << 1];
			}
