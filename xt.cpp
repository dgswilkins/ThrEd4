...
BOOL chktxh(TXHST* phst)
{
	...
	{
		if (txtmp[ind].line != phst->txp[ind].line)
			return 1;
...
void fthvars() {
	...
	rstMap(BARSAT);
	rstMap(FTHR);
	fthtyp = frmpnt->fillInfo.feather.fillType;
	brat = frmpnt->fillInfo.feather.ratio;
	fthflr = frmpnt->fillInfo.feather.minStitchSize;
	flr2 = fthflr * 2;
	fthnum = frmpnt->fillInfo.feather.count;
	xat = frmpnt->extendedAttribute;
	upfth = fthup = frmpnt->fillInfo.feather.upCount;
	dwnfth = fthdwn = frmpnt->fillInfo.feather.downCount;
	faznum = fthup + fthdwn;
	if (xat&AT_FTHBLND)
		setMap(BARSAT);
	else
...
void fritfil() {
	...
	if (seqpnt) {
		...
		chkseq(0);
		isind2++;
		if (frmpnt->extendedAttribute&AT_FTHBLND&&~(frmpnt->extendedAttribute&(AT_FTHUP | AT_FTHDWN)) != (AT_FTHUP | AT_FTHDWN)) {

			isinds[isind2].ind = isind;
			isinds[isind2].seq = I_FTH;
			isinds[isind2].cod = FTHMSK;
			isinds[isind2].col = frmpnt->fillInfo.feather.color;
			ine = xpnt - 1;
...
void fethrf() {
	...
	if (formIndex) {
		...
		deltx();
		frmpnt->type = SAT;
		frmpnt->fillInfo.feather.ratio = ini.featherRatio;
		frmpnt->fillInfo.feather.upCount = ini.featherUpCount;
		frmpnt->fillInfo.feather.downCount = ini.featherDownCount;
		frmpnt->fillInfo.feather.fillType = ini.featherFillType;
		frmpnt->fillInfo.feather.minStitchSize = ini.featherMinStitchSize;
		frmpnt->extendedAttribute &= !(AT_FTHUP | AT_FTHDWN | AT_FTHBLND);
		frmpnt->extendedAttribute |= ini.featherType;
		frmpnt->fillInfo.feather.count = ini.featherCount;
		frmpnt->lengthOrCount.stitchLength = userStitchLength;
		frmpnt->fillSpacing = StitchSpace;
		frmpnt->fillColor = actcol;
		frmpnt->fillInfo.feather.color = (actcol + 1)&COLMSK;
		frmpnt->fillType = FTHF;
		refilfn();
	}
}
...
fPOINT* insid()
{
	...
	satout(fabs(frmpnt->underlayIndent));
	if (frmpnt->underlayIndent > 0)
	{
...
void delwlk(unsigned cod)
{
	...
	{
		if ((stchs[ind].attribute&WLKFMSK) != cod)
		{
	...
	if (ine)
	{
		if (delpnt > (unsigned)hed.stitchCount)
			delpnt = hed.stitchCount;
	}
}
...
void chkuseq()
{
	...
	isind = index;
	isinds[isind2].col = frmpnt->underlayColor;
#else
	...
	if (opnt)
	{
		if (frmpnt->underlayStitchLen < MINWLK)
			frmpnt->underlayStitchLen = (float)MINWLK;
		if (frmpnt->underlayStitchLen > MAXWLK)
			frmpnt->underlayStitchLen = MAXWLK;
		ulen = frmpnt->underlayStitchLen;
...
gulabx:;
	...
	while (strt != fin)
	{
		...
		len = hypot(dif.x, dif.y);
		cnt = len / frmpnt->lengthOrCount.stitchLength;
		if (cnt > 1)
		{
			stp.x = dif.x / cnt;
			stp.y = dif.y / cnt;
...
void fnwlk(unsigned find)
{
	...
	if (frmpnt->type == LIN)
		frmpnt->type = POLI;
	if (frmpnt->extendedAttribute&AT_STRT&&frmpnt->type != LIN)
		strt = frmpnt->fillStart;
	else
		strt = 0;
	cnt = sides;
	if (frmpnt->type != LIN)
...
void undclp()
{
	clipboardRectangleSize.cx = clipboardRect.bottom = clipboardRect.left = clipboardRect.right = clpnu[0].x = clpnu[1].x = clpnu[0].y = 0;
	clipboardRectangleSize.cy = clipboardRect.top = clpnu[1].y = frmpnt->underlayStitchLen;
...
void fnund(unsigned find)
{
	...
	if (!frmpnt->underlaySpacing)
		frmpnt->underlaySpacing = ini.underlaySpacing;
	if (!frmpnt->underlayStitchLen)
		frmpnt->underlayStitchLen = ini.underlayStitchLen;
	undclp();
	setMap(ISUND);
	angclpfn();
	opnt = seqpnt;
...
void fncwlk()
{
	...
	opnt = 0;
	frmpnt->extendedAttribute |= AT_CWLK;
	if (frmpnt->satinGuideCount)
	{
		...
		sac = frmpnt->satinOrAngle.sac;
		for (ind = frmpnt->satinGuideCount; ind != 0; ind--)
		{
...
BOOL CALLBACK fthdefprc(HWND hwndlg, UINT umsg, WPARAM wparam, LPARAM lparam) {
	...
	case WM_INITDIALOG:
		...
		l_xat = ini.featherType;
		SendMessage(hwndlg, WM_SETFOCUS, 0, 0);
		sprintf_s(buf, sizeof(buf), "%.2f", ini.featherRatio);
		SetWindowText(GetDlgItem(hwndlg, IDC_DFRAT), buf);
		sprintf_s(buf, sizeof(buf), "%d", ini.featherUpCount);
		SetWindowText(GetDlgItem(hwndlg, IDC_DFUPCNT), buf);
		sprintf_s(buf, sizeof(buf), "%d", ini.featherDownCount);
		SetWindowText(GetDlgItem(hwndlg, IDC_DFDWNCNT), buf);
		sprintf_s(buf, sizeof(buf), "%.2f", ini.featherMinStitchSize / PFGRAN);
		SetWindowText(GetDlgItem(hwndlg, IDC_DFLR), buf);
		sprintf_s(buf, sizeof(buf), "%d", ini.featherCount);
		...
		SendMessage(GetDlgItem(hwndlg, IDC_FDTYP), CB_SETCURSEL, ini.featherFillType - 1, 0);
		if (l_xat&AT_FTHBLND)
			stat = BST_CHECKED;
		else
			stat = BST_UNCHECKED;
	...
	case WM_COMMAND:
		...
		case IDOK:
			...
			if (IsDlgButtonChecked(hwndlg, IDC_FDBLND))
				ini.featherType = AT_FTHBLND;
			if (IsDlgButtonChecked(hwndlg, IDC_FDUP))
				ini.featherType |= AT_FTHUP;
			if (IsDlgButtonChecked(hwndlg, IDC_FDWN))
				ini.featherType |= AT_FTHDWN;
...
void srtcol() {
	...
	FillMemory(&hst, 64, 0);
	for (ind = 0; ind < hed.stitchCount; ind++)
		hst[stchs[ind].attribute&COLMSK]++;
	ine = 0;
	...
	phi = &stchs[MAXSEQ];
	for (ind = 0; ind < hed.stitchCount; ind++)
		mvstch(&phi[hst[stchs[ind].attribute&COLMSK]++], &stchs[ind]);
	MoveMemory(&stchs, phi, sizeof(fPOINTATTRIBUTE)*hed.stitchCount);
}
...
void dubit(unsigned bit)
{
	...
	if (frmpnt->type == LIN)
		frmpnt->type = POLI;
	if (!(frmpnt->extendedAttribute&(AT_UND | AT_WALK | AT_CWLK)) && bit&(AT_UND | AT_WALK | AT_CWLK))
	{
		if (frmpnt->fillType)
			frmpnt->underlayColor = frmpnt->fillColor;
		else
			frmpnt->underlayColor = actcol;
		frmpnt->underlayStitchLen = ini.underlayStitchLen;
	}
	if (!(frmpnt->extendedAttribute&AT_UND) && bit&AT_UND)
	{
		frmpnt->underlayStitchAngle = ini.underlayAngle;
		frmpnt->underlaySpacing = ini.underlaySpacing;
	}
	cod = frmpnt->extendedAttribute&bit;
	if (cod)
		frmpnt->extendedAttribute &= (~bit);
	else
		frmpnt->extendedAttribute |= bit;
	refil();
	coltab();
...
{
	setMap(GTUANG);
	msgflt(IDS_UANG, ini.underlayAngle / PI * 180);
}

void setuspac()
{
	setMap(GTUSPAC);
	msgflt(IDS_USPAC, ini.underlaySpacing / PFGRAN);
}

void setwlkind()
{
	setMap(GTWLKIND);
	msgflt(IDS_WLKIND, ini.underlayIndent / PFGRAN);
}

void setulen()
{
	setMap(GTWLKLEN);
	msgflt(IDS_WLKLEN, ini.underlayStitchLen / PFGRAN);
}
...
void chkdaz()
{
	...
	if (!ini.daisyPetalCount)
		ini.daisyPetalCount = 1;
	len = ini.daisyDiameter*0.999;
...
BOOL CALLBACK dasyproc(HWND hwndlg, UINT umsg, WPARAM wparam, LPARAM lparam)
{
	...
	reinit:;
		...
		sprintf_s(buf, sizeof(buf), "%.2f", ini.daisyDiameter);
		SetWindowText(GetDlgItem(hwndlg, IDC_CNTLEN), buf);
		sprintf_s(buf, sizeof(buf), "%.2f", ini.daisyHoleDiameter);
		...
		sprintf_s(buf, sizeof(buf), "%d", ini.daisyPetalCount);
		SetWindowText(GetDlgItem(hwndlg, IDC_PETALS), buf);
		sprintf_s(buf, sizeof(buf), "%.2f", ini.daisyPetalLen);
		...
		SendMessage(GetDlgItem(hwndlg, IDC_DAZTYP), CB_SETCURSEL, ini.daisyBorderType, 0);
		break;
	...
	case WM_COMMAND:
		...
		case IDOK:
			...
			ini.daisyDiameter = atof(buf);
			GetWindowText(GetDlgItem(hwndlg, IDC_HOLSIZ), buf, HBUFSIZ);
			ini.daisyHoleDiameter = atof(buf);
			...
			ini.daisyPetalCount = atoi(buf);
			GetWindowText(GetDlgItem(hwndlg, IDC_PETLEN), buf, HBUFSIZ);
			ini.daisyPetalLen = atof(buf);
...
void dasyfrm() {
	...
	ini.daisyPetalCount = ini.daisyPetalCount;
	len = ini.daisyDiameter;
	elen = ini.daisyPetalLen;
	ilen = ini.daisyHoleDiameter;
	dtyp = ini.daisyBorderType;
	...
	{
		pang = PI2;
		bcnt = ini.daisyPetalCount*ini.dazicnt;
		dang = PI2 / bcnt;
	...
	pang = 0;
	bcnt = ini.daisyPetalCount*ini.dazcnt;
	pcnt = ini.dazcnt;
	if (dtyp == DHART)
	{
		pcnt = (ini.dazpcnt + 1) << 1;
		bcnt = ini.daisyPetalCount*pcnt;
	}
	dang = PI2 / bcnt;
	dangp = PI / ini.dazcnt;
	if (chku(DAZD))
	{
		frmpnt->satinGuideCount = ini.daisyPetalCount - 1;
		frmpnt->wpar = ini.daisyPetalCount*ini.dazicnt + 1;
		frmpnt->satinOrAngle.sac = adsatk(ini.daisyPetalCount - 1);
	}
	for (ind = 0; ind < ini.daisyPetalCount; ind++)
	{
		pangp = 0;
		psgacc = SEED;
		for (ine = 0; ine < pcnt; ine++)
		{
			...
			inf++;
			pang += dang;
			if (chku(DAZD) && ind != ini.daisyPetalCount - 1)
			{
				frmpnt->satinOrAngle.sac[ind].start = (ini.daisyPetalCount - ind - 1)*ini.dazicnt + 1;
				frmpnt->satinOrAngle.sac[ind].finish = inf;
			}
		}
	}
	if (chku(DAZHOL))
...
void dmprec(OREC** recs, unsigned cnt)
{
	...
	for (ind = 0; ind < cnt; ind++)
	{
		sprintf_s(msgbuf, sizeof(msgbuf), "%4d off: %4d at: %08x frm: %4d typ: %d col: %2d st: %5d fin: %5d\n",
			ind,
			recs[ind] - (OREC*)bseq,
			stchs[recs[ind]->start].attribute,
			recs[ind]->frm,
			recs[ind]->typ,
			recs[ind]->col,
			recs[ind]->start,
...
void fsort()
{
	...
	{
		if ((stchs[ind].attribute&SRTMSK) != at)
		{
			...
			at = stchs[ind].attribute&SRTMSK;
		}
	}
	recs[rind].epnt = &stchs[hed.stitchCount - 1];
	recs[rind].finish = hed.stitchCount;
	rind++;
	...
	for (ind = 0; ind < hed.stitchCount; ind++)
	{
		...
		if (fcod == (at&FRMSK))
		{
			...
			case 6:	//border
				...
				if (frmpnt->fillColor == frmpnt->borderColor)
				{
					if (!(*lmap&M_BRD))
						flim->bstrt = ind;
					*lmap |= M_BRD;
...
void dmpat()
{
	...
	duatf(0);
	at = stchs[0].attribute;
	for (ind = 1; ind < hed.stitchCount; ind++)
	{
		if (at != stchs[ind].attribute)
		{
			duatf(ind);
			at = stchs[ind].attribute;
		}
...
void fdelstch()
{
	...
	fcod = (closestFormToCursor << FRMSHFT);
	ine = 0;
	bcol = frmpnt->borderColor&COLMSK;
	tapcol = frmpnt->borderColor >> 4;
	for (ind = 0; ind < hed.stitchCount; ind++)
	{
		...
		{
			col = at&COLMSK;
			if (col == frmpnt->fillColor)
			{
				tmap |= M_FCOL;
				fstrts.fcol = ine;
			}
			if (col == frmpnt->fillInfo.feather.color)
			{
				tmap |= M_FTHCOL;
				fstrts.fthcol = ine;
			}
	...
	ine = 0;
	if (!(tmap&M_ECOL))
		fstrts.ecol = hed.stitchCount;
	if (!(tmap&M_FTHCOL))
		fstrts.fthcol = hed.stitchCount;
	if (!(tmap&M_FCOL))
		fstrts.fcol = hed.stitchCount;
	if (frmpnt->edgeType)
	{
		if (frmpnt->edgeType == EGAP)
		{
			if (!(tmap&M_AP))
			{
				if (tmap&M_APCOL)
		...
		if (!(tmap&M_BRD))
		{
			if (tmap&M_ECOL)
				...
				fstrts.brd = hed.stitchCount;
		}
	}
	if (frmpnt->fillType || (tmap&(M_WALK | M_UND | M_CWLK)))
	{
		if (!(tmap&M_FIL))
		{
			if (tmap&M_FCOL)
...
void duint(unsigned off, unsigned cod)
{
	...
	if (itf.coloc > itf.sloc)
	{
		...
		MoveMemory(&itf.histch[itf.oloc], &stchs[itf.sloc], sizeof(fPOINTATTRIBUTE)*cnt);
		itf.sloc += cnt;
		itf.oloc += cnt;
	}
	if (frmpnt->extendedAttribute&AT_STRT)
	{
		if (!setMap(DIDSTRT))
			itf.oloc += gucon(currentFormVertices[frmpnt->fillStart], iseq[isinds[itf.pins].ind], itf.oloc + off, cod);
	}
...
BOOL isfil()
{
	...
	if (frmpnt->edgeType)
		return 1;
	if (frmpnt->extendedAttribute&(AT_CWLK | AT_WALK | AT_UND))
		return 1;
	return 0;
}
...
void chkend(unsigned off, unsigned cod)
{
	if (isfil())
	{
		...
		if (frmpnt->extendedAttribute&AT_END)
			itf.oloc += gucon(iseq[isind - 1], currentFormVertices[frmpnt->fillEnd], itf.oloc + off, cod);
	}
}
...
void intlv()
{
	...
	itf.laycod = ((frmpnt->attribute&FRMLMSK) << (LAYSHFT - 1)) | (closestFormToCursor << FRMSHFT);
	rstMap(DIDSTRT);
	if (hed.stitchCount)
	{
		off = MAXSEQ;
		...
		chkend(MAXSEQ, cod);
		if (hed.stitchCount&&itf.sloc < (unsigned)hed.stitchCount - 1)
		{
			ine = hed.stitchCount - itf.sloc;
			MoveMemory(&stchs[itf.oloc + MAXSEQ], &stchs[itf.sloc], sizeof(fPOINTATTRIBUTE)*ine);
			itf.oloc += ine;
		}
		MoveMemory(stchs, itf.histch, sizeof(fPOINTATTRIBUTE)*itf.oloc);
	}
	else
	{
		off = 0;
...
void uangfn(unsigned find, float p_ang)
{
	...
	if (frmpnt->extendedAttribute&AT_UND)
	{
		frmpnt->underlayStitchAngle = p_ang;
		refilfn();
	}
}
...
void flenfn(unsigned find, float len)
{
	closestFormToCursor = find;
	fvars(closestFormToCursor);
	if (frmpnt->fillType && !isclp(find))
	{
		frmpnt->lengthOrCount.stitchLength = len;
		refilfn();
	}
}
...
void findfn(unsigned find, float idnt)
{
	closestFormToCursor = find;
	fvars(closestFormToCursor);
	frmpnt->underlayIndent = idnt;
	if (frmpnt->extendedAttribute&(AT_UND | AT_WALK))
		refilfn();
}
...
void fangfn(unsigned find, float p_ang)
{
	closestFormToCursor = find;
	fvars(closestFormToCursor);
	if (frmpnt->type == POLI&&frmpnt->fillType)
	{
		...
		case ANGF:
			...
			frmpnt->angleOrClipData.fang = p_ang;
			break;
		...
		case	ANGCLPF:
			...
			frmpnt->satinOrAngle.angle = p_ang;
			break;
		}
		refilfn();
	}
...
void blenfn(unsigned find, float len)
{
	closestFormToCursor = find;
	fvars(closestFormToCursor);
	if (frmpnt->edgeType && !iseclp(find))
	{
		frmpnt->lengthOrCount.stitchLength = len;
		refilfn();
	}
...
void fwidfn(unsigned find, float len)
{
	...
	rat = len / (frmpnt->rectangle.right - ref);
	for (ind = 0; ind < sides; ind++)
...
void fhifn(unsigned find, float len)
{
	...
	rat = len / (frmpnt->rectangle.top - ref);
	for (ind = 0; ind < sides; ind++)
...
void dutxtfil()
{
	if (!ini.textureHeight)
		ini.textureHeight = ITXHI;
	if (!ini.textureWidth)
		ini.textureWidth = ITXWID;
	if (!ini.textureSpacing)
		ini.textureSpacing = (float)ITXSPAC;
	if (!ini.textureEditorSizePixels)
		ini.textureEditorSizePixels = ITXPIX;
	angfrm.sides = 0;
	setMap(TXTRED);
	setMap(ZUMED);
	rstMap(WASPAT);
	...
	if (chkMap(WASTXBAK))
	{
		redtbak();
		if (!tscr.fhi)
			tscr.fhi = ini.textureHeight;
		if (!tscr.spac)
			tscr.spac = ini.underlaySpacing;
		if (!tscr.wid)
			tscr.wid = ini.textureWidth;
		setMap(LASTXBAK);
		rstMap(TXBDIR);
	}
	else
...
void txt2pix(TXPNT txp, POINT* pixp)
{
	...
	pixp->x = (txp.line*tscr.spac + tscr.xof) / tscr.ed2px;
}
...
void txrct2rct(TXTRCT txr, RECT* rct)
{
	...
	txp.y = txr.top;
	txt2pix(txp, &tpnt);
	rct->left = tpnt.x - ini.textureEditorSizePixels;
	rct->top = tpnt.y - ini.textureEditorSizePixels;
	txp.line = txr.rit;
	txp.y = txr.bot;
	txt2pix(txp, &tpnt);
	rct->right = tpnt.x + ini.textureEditorSizePixels;
	rct->bottom = tpnt.y + ini.textureEditorSizePixels;
}

void ed2px(fPOINT ped, POINT* px)
{
	px->x = ped.x / tscr.ed2px;
	px->y = stitchWindowClientRect.bottom - ped.y / tscr.ed2px;
}
...
void chktx()
{
	...
	for (ind = 0; ind < tscr.ind; ind++)
	{
		if (txtmp[ind].line <= tscr.lins&&txtmp[ind].y < tscr.fhi)
		{
...
void drwtxtr()
{
	...
	{
		tscr.xof = 0;
		tscr.ed2px = wid2 / stitchWindowClientRect.bottom;
		ofy = (stitchWindowClientRect.bottom - tscr.fhi / tscr.ed2px) / 2;
	}
	else
	{
		tscr.ed2px = tscr.fhi * 2 / stitchWindowClientRect.bottom;
		ofy = stitchWindowClientRect.bottom >> 2;
		tscr.xof = (stitchWindowClientRect.right*tscr.ed2px - (tscr.lins + 2)*tscr.spac) / 2;
	}
	tscr.top = ofy;
	tscr.bot = stitchWindowClientRect.bottom - ofy;
	tscr.hi = tscr.bot - tscr.top;
	tscr.mhi = stitchWindowClientRect.bottom >> 1;
	tscr.ehi = stitchWindowClientRect.bottom*tscr.ed2px;
	tscr.yof = (tscr.ehi - tscr.fhi) / 2;
	SetROP2(StitchWindowMemDC, R2_XORPEN);
	SelectObject(StitchWindowMemDC, grdPen);
	cnt = tscr.fhi / ini.gridSize + 1;
	txp.line = 0;
	xlin[0].x = 0;
	...
	{
		dutxtx(cloxlst[ind], ini.textureEditorSizePixels);
		dutxtx(cloxlst[ind], ini.textureEditorSizePixels << 1);
	}
...
BOOL px2txt(POINT pof, TXPNT* txrec)
{
	...
	px2ed(pof, &epnt);
	txrec->line = (epnt.x - tscr.xof) / tscr.spac + 0.5;
	if (txrec->line > tscr.lins)
		return 0;
	if (txrec->line < 1)
		return 0;
	if (pof.y > tscr.top)
...
void txtrup()
{
	...
	if (rstMap(TXTMOV))
	{
		...
		of.y -= cloxref.y;
		bakp = abs(of.x);
		tof.line = bakp*tscr.ed2px / tscr.spac + 0.5;
		if (of.x < 0)
			tof.line = -tof.line;
		...
		if (tflt < 0)
			tof.y -= tflt;
		ind = txrct.lft + tof.line - 1;
		if (ind < 0)
			tof.line -= ind;
		ind = txrct.rit + tof.line - tscr.lins;
		if (ind > 0)
			tof.line -= ind;
		for (ind = 0; ind < cloxcnt; ind++)
		{
...
void txtclp()
{
	...
	if (hClpMem)
	{
		clipboardFormData = (FORMCLIP*)GlobalLock(hClpMem);
		if (clipboardFormData) {
			if (clipboardFormData->clipType == CLP_FRM) {
				frmpnt = &clipboardFormData->form;
				frmcpy(&angfrm, frmpnt);
				MoveMemory(&angflt, &frmpnt[1], sizeof(fPOINT)*frmpnt->sides);
				angfrm.vertices = angflt;
				rstMap(TXTLIN);
				setMap(TXTCLP);
				setMap(TXTMOV);
...
{
	frmpnt->type = POLI;
	frmpnt->fillInfo.texture.lines = tscr.lins;
	frmpnt->fillInfo.texture.height = tscr.fhi;
	frmpnt->fillSpacing = tscr.spac;
	frmpnt->lengthOrCount.stitchLength = ini.userStitchLength;
	frmpnt->maxFillStitchLen = ini.maxStitchLength;
	frmpnt->minFillStitchLen = ini.minStitchLength;
	frmpnt->fillColor = actcol;
	refilfn();
}
...
void deltx()
{
	...
	cnt = frmpnt->fillInfo.texture.count;
	if (txad&&istx(closestFormToCursor) && cnt)
	{
		ind = frmpnt->fillInfo.texture.index;
		...
		txad -= frmpnt->fillInfo.texture.count;
		frmpnt->fillInfo.texture.count = 0;
	}
}
...
void nutx()
{
	...
	frmpnt->fillInfo.texture.index = ins;
	frmpnt->fillInfo.texture.count = tscr.ind;
}
...
void dutxmir()
{
	...
	while (ind >= 0)
	{
		txtmp[ine].line = tscr.lins - txtmp[ind].line + 1;
		txtmp[ine].y = txtmp[ind].y;
		ine++;
		ind--;
	}
	...
	if (tscr.ind)
	{
		...
		if (cloxcnt)
		{
			for (ind = 0; ind < cloxcnt; ind++)
			{
				...
				cnt = (tp->y + siz2) / ini.gridSize;
				tp->y = cnt*ini.gridSize;
			}
		}
		else
		{
...
void setxt()
{
	...
	frmpnt->wpar = 0;
	setMap(TXFIL);
	clipboardRectangleSize.cx = frmpnt->fillSpacing;
	clipboardRectangleSize.cy = frmpnt->fillInfo.texture.height;
	txsegs = (RNGCNT*)&rmap;
	pbak = &txpnts[frmpnt->fillInfo.texture.index];
	FillMemory(txsegs, frmpnt->fillInfo.texture.lines * sizeof(RNGCNT), 0);
	pbak = &txpnts[frmpnt->fillInfo.texture.index];
	cnt = frmpnt->fillInfo.texture.count;
	if (cnt)
	{
		...
		{
			txsegs[pbak[ind].line - 1].lin = ind;
			txsegs[pbak[ind].line - 1].cnt++;
		}
	}
}
...
void rtrtx()
{
	...
	fvars(closestFormToCursor);
	tps = &txpnts[frmpnt->fillInfo.texture.index];
	tscr.ind = frmpnt->fillInfo.texture.count;
	MoveMemory(txtmp, tps, sizeof(TXPNT)*tscr.ind);
	tscr.fhi = frmpnt->fillInfo.texture.height;
	tscr.spac = frmpnt->fillSpacing;
	tscr.lins = frmpnt->fillInfo.texture.lines;
...
void nudsiz()
{
	...
	if (flg)
	{
		...
		{
			flg = 0;
			if (dsgnsiz.x > ini.hoopSizeX)
			{
				ini.hoopSizeX = dsgnsiz.x*1.05;
				unzoomedRect.x = ini.hoopSizeX;
				flg = 1;
			}
			if (dsgnsiz.y > ini.hoopSizeY)
			{
				ini.hoopSizeY = dsgnsiz.y*1.05;
...
void setshft()
{
	...
	if (tscr.ind) { lin = txtmp[tscr.ind - 1].line; }
...
void setclpspac()
{
	msgflt(IDS_CLPSPAC, ini.clipboardOffset / PFGRAN);
	setMap(NUMIN);
	setMap(SCLPSPAC);
	numWnd();
}
...
{
	if (bc->clp == fp->angleOrClipData.clp - clipboardPoints)
...
{
	if (bc->clp == fp->borderClipboardData - clipboardPoints)
		bc->clp += fp->nclp;
	else
		bc->at |= BADCLP;
}
...
void repclp()
{
	...
	for (ind = 0; ind < formIndex; ind++)
	{
		...
		if (isclp(ind))
		{
			loc = fp->angleOrClipData.clp - clipboardPoints;
			if (loc + fp->lengthOrCount.clipCount < clpad)
			{
				MoveMemory(&tclps[cnt], fp->angleOrClipData.clp, sizeof(fPOINT)*fp->lengthOrCount.clipCount);
				fp->angleOrClipData.clp = &clipboardPoints[cnt];
		...
		if (iseclp(ind))
		{
			loc = fp->borderClipboardData - clipboardPoints;
			if (loc + fp->nclp < clpad)
			{
				MoveMemory(&tclps[cnt], fp->borderClipboardData, sizeof(fPOINT)*fp->nclp);
				fp->borderClipboardData = &clipboardPoints[cnt];
				cnt += fp->nclp;
			}
			else
			{
